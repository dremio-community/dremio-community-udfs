  ...
  public void eval() {
    if (org.sheinbergon.dremio.udf.gis.util.GeometryHelpers.isHolderSet(binaryInput)
      ...
      if (geom instanceof org.locationtech.jts.geom.LineString) {
        ...
        int n = indexParam.value;
        // 1-based index, negative counts from end
        int idx = (n >= 0) ? (n - 1) : (line.getNumPoints() + n);
        if (idx >= 0 && idx < line.getNumPoints()) {
          org.locationtech.jts.geom.Point pt = line.getPointN(idx);
