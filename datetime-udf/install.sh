#!/usr/bin/env bash
# install.sh — Deploy the Dremio Date/Time UDF to a running Dremio instance
#
# Usage:
#   ./install.sh [OPTIONS]
#
...
if $PREBUILT; then
  JAR_PATH=$(find "$SCRIPT_DIR/jars" -name "dremio-datetime-udf-*.jar" | head -1)
  ...
  JAR_PATH=$(find "$SCRIPT_DIR/target" -name "dremio-datetime-udf-*.jar" ! -name "*sources*" | head -1)
...
echo ""
ok "Date/Time UDF installed successfully"
echo ""
echo "  Test with:"
echo "    SELECT DT_FISCAL_YEAR(CAST('2024-01-15' AS DATE), 7)"
echo "    SELECT DT_BIZDAYS_BETWEEN(CAST('2024-01-01' AS DATE), CAST('2024-01-31' AS DATE))"
echo "    SELECT DT_QUARTER_END(CAST('2024-07-15' AS DATE))"
echo "    SELECT DT_FORMAT(CAST('2024-03-15' AS DATE), '%A %B %d, %Y')"
echo ""
